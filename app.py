...
def initialize_recommendation_system():
    ...
    try:
        ...
        csv_path = os.path.join(base_dir, "books_with_simple_categories.csv")
...
def retrieve_semantic_recommendation(query, top_k=5):
    ...
    try:
        ...
        recs = db_books.similarity_search_with_score(query, k=top_k * 2)
        books_list = []
        score_map = {}
        
        for rec, distance in recs:
            ...
            if len(isbn_digits) >= 10:  # Valid ISBN length
                isbn = int(isbn_digits)
                books_list.append(isbn)
                
                similarity = max(0, min(100, round((1 - distance / 2) * 100, 1)))
                score_map[isbn] = similarity
        ...
        for _, book in recommended_books.iterrows():
            recommendations.append({
                ...
                'thumbnail': book['thumbnail'] if pd.notna(book['thumbnail']) else '',
                ...
                'similarity': score_map.get(int(book['isbn13']), 0)
            })
